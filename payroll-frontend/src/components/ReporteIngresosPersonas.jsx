...
const ReporteIngresosPersonas = () => {
  ...
  const generarReporte = () => {
    ...
    console.log(`Solicitando reporte de personas desde ${inicio} hasta ${fin}`);

    // Llamar al backend
    reservaService
      .getReporteIngresosPorPersonas(inicio, fin)
      .then((response) => {
        console.log("Datos recibidos del reporte de personas:", response.data);
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ mb: 3, display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        >
          {loading ? 'Generando...' : 'Show Reporte'}
        </Button>
      </Box>
      ...
      <TableContainer component={Paper}>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell>
                <Typography fontWeight="bold">Grupo de Personas</Typography>
              </TableCell>
              {meses.map((mes) => (
